...
/**
 * A SECOND {@link MultiTenantResource} fixture that {@link TestTenantMembershipResolver}
 * deliberately does NOT support, used to exercise the "no TenantMembershipResolver registered"
 * branch of {@code BaseEntityServiceImpl#buildMultiTenantCondition} (which logs a warning and
 * leaves the query unfiltered, rather than denying access).
 */
public class UnresolvedMultiTenantTestEntity implements MultiTenantResource {
    private long id;
    ...
    public UnresolvedMultiTenantTestEntity() {
    }

    public UnresolvedMultiTenantTestEntity(UnresolvedMultiTenantTestEntity source) {
        this.id = source.id;
